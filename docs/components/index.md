...
**`JSWidgetFont`**（见 [`types.d.ts`](../dts/types.d.ts)）：
...
定义于 `components.d.ts`，多数标签通过交叉类型继承。
...
`number` 或对象：`{ horizontal?, vertical?, top?, bottom?, leading?, trailing?, left?, right? }`（见 [`types.d.ts`](../dts/types.d.ts)）。
